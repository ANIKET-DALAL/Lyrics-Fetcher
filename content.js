...
function gotMessage(message, sender, sendResponse) {
        if(message.txt === 'hello' && $("h1.title")) {
            ...
            if(title!=''){
                ...
                channelName = $(".style-scope ytd-video-owner-renderer .style-scope.ytd-channel-name .yt-simple-endpoint.style-scope.yt-formatted-string").first().text().trim()
            ...
            if(parseInfo(title).artist==''){
                ...
                sendResponse({artist: parseInfo(title).artist, track: parseInfo(title).track})
            }
        }
...
function parseInfo(artistTitle) {
    ...
    track = artistTitle.substr(separator.startIndexTrack, separator.endIndexTrack);
...
function findSeparator(str) {
    ...
    let separators = [' - ', ' – ', '-', '–', ':'];
    let separators_2 = ['ft.', ' ft. ', 'Ft.', ' Ft. ', 'feat', ' feat ', 'FEAT.', ' FEAT. ', '|', ' | ', '&', ' & ']
    let separators_3 = [',',' , ', 'ft.', ' ft. ', 'Ft.', ' Ft. ', 'feat', ' feat ', 'FEAT.', ' FEAT. ', '|', ' | ', '&', ' & ', ' x ', ' X ']
    let bracket = ['(']
    let backIndex = str.indexOf(bracket[0])
    ...
    if (startIndexTrack > -1 && (startIndexTrack<backIndex || backIndex==-1)) {
        return { endIndexArtist: endIndexArtist, startIndexTrack: startIndexTrack, endIndexTrack: endIndexTrack };
    }
...
function cleanTrack(track) {
    ...
    track = track.replace(/\s*\[[^\]]+\]$/, ''); // [whatever]
    track = track.replace(/\s*\([^\)]*version\)$/i, ''); // (whatever version)
    track = track.replace(/\s*\(Prod*[^\)]+\)$/i, ''); // (whatever version)
    track = track.replace(/\s*\.(avi|wmv|mpg|mpeg|flv)$/i, ''); // video extensions
    track = track.replace(/\s*(LYRIC VIDEO\s*)?(lyric video\s*)/i, ''); // (LYRIC VIDEO)
